...
def create_evidence_bundle(
    ...
    metadata = {
        ...
        "started_at_utc": evidence.get("started_at"),
        "finished_at": evidence.get("finished_at"),
        "finished_at_utc": evidence.get("finished_at"),
        ...
        "commands_executed": evidence.get("command_count", len(evidence.get("commands", {}))),
        ...
        "commands_failed": evidence.get(
            "failed_command_count",
            len([result for result in evidence.get("commands", {}).values() if result.get("exit_code") not in (0, None)]),
        ),
        "file_change_metrics": evidence.get("file_change_metrics", {}),
        "files_created": evidence.get("file_change_metrics", {}).get("files_created", 0),
        "files_modified": evidence.get("file_change_metrics", {}).get("files_modified", 0),
        "files_deleted": evidence.get("file_change_metrics", {}).get("files_deleted", 0),
        "lines_added": evidence.get("file_change_metrics", {}).get("lines_added", 0),
        "lines_removed": evidence.get("file_change_metrics", {}).get("lines_removed", 0),
        "browser_qa_report": evidence.get("browser_qa_report"),
        "browser_qa_verdict": evidence.get("browser_qa_verdict"),
        "browser_qa_mode": evidence.get("browser_qa_mode"),
        "browser_qa_summary": evidence.get("browser_qa_summary"),
        "backend_audit_report": evidence.get("backend_audit_report"),
        "risk_level": (risk_summary or {}).get("risk_level"),
        "qa_verdict": (risk_summary or {}).get("verdict"),
        "research_requests": evidence.get("research_requests", 0),
